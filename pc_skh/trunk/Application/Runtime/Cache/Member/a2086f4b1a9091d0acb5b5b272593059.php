<?php if (!defined('THINK_PATH')) exit();?><!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <meta http-equiv="X-UA-Compatible" content="ie=edge">
  <meta content="<?php echo ($seoarr['69942757']['KEYWORDS']); ?>" name="keywords">
  <meta content="<?php echo ($seoarr['69942757']['DESCRIPTION']); ?>" name="description">
  <title><?php echo ($seoarr['69942757']['TITLE']); ?></title>
  <!-- 公共样式 -->
  <link rel="stylesheet" href="/Public/Common/css/header_ft.css">
  <!-- 当前样式 -->
  <link rel="stylesheet" href="/Public/css/focus/intro.css">
</head>
<body>
  <!-- 头部start -->
  
  <!-- 头部start -->
  <div class="header">
      <div class="h_top">
        <div class="w1200">
          <div class="fl"><span>欢迎来到北京圣康华</span></div>
          <div class="fr tr"><span>客服热线：400-656-2020</span></div>
        </div>
      </div>
      <div class="h_bottom clear">
        <div class="w1200">
          <a href="/" class="fl logo"><img src="/Public/Common/images/logo.png" alt=""></a>
          <div class="fl">
            <ul id="list">
              <li class="tab_nav"><a href="/">首页</a></li>
              <li class="tab_nav">
                <a href="/Focus/11126">知名专家<i></i></a>
                <!-- <ol class="tab_con"> -->
                  <!-- <a href="/Focus/doc/type_id/11119"><li>特需专家</li></a>
                  <a href="/Focus/doc/type_id/11120"><li>会诊专家</li></a>
                  <a href="/Focus/doc/type_id/11121"><li>眼底病科</li></a>
                  <a href="/Focus/doc/type_id/11122"><li>白内障科</li></a>
                  <a href="/Focus/doc/type_id/11123"><li>眼外伤科</li></a>
                  <a href="/Focus/doc/type_id/11124"><li>神经眼科</li></a>
                  <a href="/Focus/doc/type_id/11125"><li>眼视光与功能视觉</li></a> -->
                <!-- </ol> -->
              </li>
              <li class="tab_nav">
                <a href="#">就诊服务<i></i></a>
                <ol class="tab_con">
                  <a href="/Service/minSug"><li>糖网</li></a>
                  <a href="/Service/cataract"><li>白内障</li></a>
                  <a href="/Service/fundusDiseases"><li>眼底病</li></a>
                  <a href="/Service/optometry"><li>眼视光</li></a>
                  <!-- <a href="#"><li>眼整形</li></a> -->
                </ol>
              </li>
              <li class="tab_nav">
                <a href="#">特色服务<i></i></a>
                <ol class="tab_con">
                  <a href="/KeyService/consultationCentre"><li>远程眼科会诊中心</li></a>
                  <a href="/KeyService/imgDiagnosis"><li>眼科影像诊断中心</li></a>
                  <!-- <a href="#"><li>糖尿病眼病远程医疗服务中心</li></a>
                  <a href="#"><li>视觉医疗质控管理服务中心</li></a> -->
                </ol>
              </li>
              <li class="tab_nav">
                <a href="#">聚焦圣康华<i></i></a>
                <ol class="tab_con">
                  <a href="/Focus/intro"><li>圣康华简介</li></a>
                  <a href="/Focus/news"><li>新闻动态</li></a>
                </ol>
              </li>
              <li class="tab_nav">
                <a href="#">联系我们<i></i></a>
                <ol class="tab_con">
                  <a href="/Contact/guide"><li>患者就医指南</li></a>
                  <!-- <a href="#"><li>联系我们</li></a> -->
                </ol>
              </li>
            </ul>
          </div>
          <!-- <div class="fr search_box">
            <input class="search" type="text" name="" value="" placeholder="搜索医生服务">
            <input type="button" name="" value="" class="searchButton">
          </div> -->
        </div>
      </div>
    </div>
  <!-- 头部end -->

  <!-- 头部end -->
  <div class="banner">
    <!-- <img src="/Public/images/focus/intro/banner.jpg" alt=""> -->
  </div>
  <div class="con1 con">
    <div class="w1200">
      <div class="w1080">
        <h2>品<em>质圣康</em>华</h2>
        <p>北京圣康华眼科医院是一家由眼科学者发起创建，集医、教、研为一体的学院型、平台型眼科专科医院，是北京市医保定点专科医院，也是北京地区少数拥有外事管理资质的医疗机构之一。</p>
        <p>医院定位于开展国际先进的医疗项目，开创新的医疗技术，解决复杂疑难眼病的医疗问题及对人群视觉健康造成重大危害的疾病的医疗问题，为眼病患者提供高质量的眼科医疗服务。</p>
        <p>作为一家平台型专科医疗机构，北京圣康华眼科医院的核心医疗团队以医生合伙人机制进行建设。借助地处首都核心区的地理资源优势和国家医改的时机，医院凝聚了一大批来自国内外的优秀眼科学者和学界领袖，由此形成了医院坚实的医疗、科研和教学力量。保证了医院在相关专业领域的领导性医疗服务能力。</p>
        <p>医院设有四个重点专业科室，包括眼底病与糖尿病眼病医疗中心、青光眼、白内障、功能视觉医疗中心。医院接受国内外医疗机构的眼科医疗转诊，为来自世界各地的眼病患者提供高质量的眼科医疗服务。</p>
        <p>与此同时，医院设综合眼科、听觉康复科和内科等临床科室；综合眼科为广大眼病患者提供高质量的基础眼科医疗服务；听觉康复科可为有视听障碍患者提供听觉康复专业服务；内科以心血管病和糖尿病为重点专业，为医院的眼科医疗工作提供内科专业支持保障服务。</p>
        <p>北京圣康华眼科医院“远程眼科影像诊断与会诊服务中心”是医院的网络医院专业服务平台，可为国内外医疗机构或眼科医生提供独立第三方眼科影像诊断服务，也可为患者个人提供眼科医疗咨询及会诊服务。</p>
        <p>医院通过广泛的国际资源整合与合作，开展具有国际水平的眼科临床与公共卫生研究工作，先后获得多项国际组织支持。医院在糖尿病视网膜病变医疗方面的研究工作成就，赢得了国际同行的广泛认可，成为国际糖尿病联合会的全球试点推广项目。医院与美国威斯康辛大学眼科中心专家合作建立的“糖尿病视网膜病变眼科影像诊断与眼科医疗质量保障服务中心”（简称“糖尿病视网膜病变影像诊断中心”），为国际糖尿病联合会的有关项目提供人才培训和糖尿病眼科医疗技术支持与质量管理服务。</p>
        <p>
          <img src="/Public/images/focus/intro/pic1.jpg" alt="">
          <img src="/Public/images/focus/intro/pic2.jpg" alt="">
          <img src="/Public/images/focus/intro/pic3.jpg" alt="">
        </p>
      </div>
    </div>
  </div>
  <div class="con2 con">
    <div class="w1200">      
      <div class="w1080">
        <h2>圣康<em>华专家</em>团</h2>
        <dl>
          <dt class="fl"><img src="/Public/images/focus/intro/pic4.jpg" alt=""></dt>
          <dd class="fr">圣康华为提高就医质量，组建了一支实力强大的专家团队常年坐诊。为了能够在技术上保持先进，在这批拥有丰富临床经验、管理经验以及高尚医德医风的资深专家带领下，圣康华不仅将服务品质推陈出新，更将越来越多的前沿医疗技术运用于临床，各大主力科室的技术力量与服务水平都不断完善提升，全面保障医疗品质及医疗安全。</dd>
        </dl>
      </div>
    </div>
  </div>
  <div class="con3 con">
    <div class="w1080">
      <h2>圣康<em>华诊疗</em>服务</h2>
      <p>医院设有四个重点专业科室，包括眼底病与糖尿病眼病医疗中心、青光眼、白内障、功能视觉医疗中心。医院接受国内外医疗机构的眼科医疗转诊，为来自世界各地的眼病患者提供高质量的眼科医疗服务。</p>
      <img src="/Public/images/focus/intro/pic5.jpg" alt="">
    </div>
  </div>
  <div class="con4 con">
    <div class="w1080">
      <h2>圣康<em>华服务</em>理念</h2>
      <ul>
        <li>
          <p class="title">患者至上</p>
          <img src="/Public/images/focus/intro/pic6.jpg" alt="">
          <p class="desc">看病就是看专家，患者可根据自己的情况，自主选择专家看病。我院专家都是针对各种疾病各有深入研究，特为广大患者开启就医绿色通道。</p>
        </li>
        <li>
          <p class="title">专病专治</p>
          <img src="/Public/images/focus/intro/pic7.jpg" alt="">
          <p class="desc">我院为提高患者就诊质量，特聘资深专家亲诊，并拥有与时俱进的诊疗技术和设备，针对患者实行个性化诊疗方案及治疗。</p>
        </li>
      </ul>
    </div>
  </div>
  <!-- 底部start -->
    
  <!-- 底部start -->
    <div class="footer">
      <div class="ft_top clear">
        <div class="w1200">
          <ul>
            <li><a href="">诊疗项目</a></li>
            <li><a href="/Service/minSug">糖网</a></li>
            <li><a href="/Service/cataract">白内障</a></li>
            <li><a href="/Service/fundusDiseases">眼底病</a></li>
            <li><a href="/Service/optometry">眼视光</a></li>
          </ul>
          <ul>
            <li><a href="">便捷服务</a></li>
            <li><a href="/Contact/guide/">来院路线</a></li>
            <li><a href="http://pbt.zoosnet.net/LR/Chatpre.aspx?id=PBT44701575&lng=cn&p=%e9%a2%84%e7%ba%a6%e4%b8%93%e5%ae%b6">在线咨询</a></li>
            <li><a href="http://pbt.zoosnet.net/LR/Chatpre.aspx?id=PBT44701575&lng=cn&p=%e9%a2%84%e7%ba%a6%e4%b8%93%e5%ae%b6">预约挂号</a></li>
          </ul>
          <ul>
            <li><a href="">聚焦圣康华</a></li>
            <li><a href="/Focus/intro">圣康华简介</a></li>
            <li><a href="/Focus/news">媒体报道</a></li>
          </ul>
          <div class="text">
            <p class="title">服务热线</p>
            <strong>400-656-2020</strong>
            <p class="range">周一至周五 8:00-17:00</p>
          </div>
        </div>
      </div>
      <div class="ft_bottom">Copyright © 2016 北京远程新华网络技术有限公司 版权所有 京ICP备14009444-1</div>
    </div>
    <div style="width:0; height:0px; overflow:hidden;"><script src="https://s19.cnzz.com/z_stat.php?id=1271236175&web_id=1271236175" language="JavaScript"></script><div>
    <script language="javascript" src="http://pbt.zoosnet.net/JS/LsJS.aspx?siteid=PBT44701575&float=1&lng=cn"></script>

    <script>
      var _hmt = _hmt || [];
      (function() {
        var hm = document.createElement("script");
        hm.src = "https://hm.baidu.com/hm.js?4b0a00a669721496ca7a8bc6d4cf58f9";
        var s = document.getElementsByTagName("script")[0]; 
        s.parentNode.insertBefore(hm, s);
      })();
    </script>

  <!-- 底部end -->

  <!-- 底部end -->
  <script src="/Public/Common/js/jquery.js" charset="utf-8"></script>
  <script src="/Public/Common/js/header.js" charset="utf-8"></script>
</body>
</html>