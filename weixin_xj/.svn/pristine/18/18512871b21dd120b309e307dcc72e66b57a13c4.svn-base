...
{
	$post_ip = get_client_ip(0,true);
	if($post_ip == "0.0.0.0" || $post_ip == 0 || !$post_ip ){
		return '00004';
	}
	...
	{
		$token = A_token();
		$url = C("PATH_URL")."/interface/yc_member/check_message_limit.aspx?access_token=".$token;
		$curlPost = array
		(
			"link_phone"=>"$link_mobile",
			"sms_type"=>"$type"
		);
		$json=json_encode($curlPost);
		$error_code = json_decode(post_json($url,$json), true);
		$error_code = $error_code['error_code'];
		if($error_code == "0"){	

			$cache = new \Think\Cache\Driver\Memcache(); 
			$StatKey=md5('stat'.$post_ip.$link_mobile.$type);
			$LastKey=md5('last'.$post_ip.$link_mobile.$type);
			$stat=$cache->get($StatKey);
			$last=$cache->get($LastKey);
		  	if($stat&&$stat>=5){
	             //return '00003-'.$stat."-".$last;
	             return '00005';
	         }

	         if($last&&$last>(time()-60)){
	         	//return '00004-'.$stat."-".$last;
	         	return '00003';
	         }

			$token = A_token();
			$url = C("PATH_URL")."/interface/app_jwsh/get_sms_voice_code.aspx?access_token=".$token;
			$curlPost = array
			(
				"sms_type"=>"$type",
				"link_phone"=>"$link_mobile",
			...
			if(isset($jdata['error_code'])){
				$error_code = $jdata['error_code'];
			}
			$cache->set($StatKey,$stat+1,"3600"); 
	        $cache->set($LastKey,time(),"600"); 
	    }
...
/**
	...
	function is_ceshi($openid)
	{
		...
		$openid_member = array(
			...
			"oEMR8jlqfy6FjLOD1RSJTe65a5JA",//焦亚磊
		);
