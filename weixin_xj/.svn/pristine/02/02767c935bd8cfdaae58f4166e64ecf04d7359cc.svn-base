...
@$consult_title = ($subject=="在线咨询")?"挂号费":"订单支付";
@$status =$json['status'];
@$doc_id =$json['doc_id'];
@$hz_id =$json['hz_id'];
...
<head>
		...
		WeixinJSBridge.invoke(
			...
			function(res){
				...
				if(res.err_msg=="get_brand_wcpay_request:ok"){
					...
					{
						//去到会员的咨询页面
						window.location.href="/weixin/index.php?m=Member&c=Consult&a=onlin_play&out_trade_no=<?php echo $out_trade_no;?>&outtradeno=<?php echo $outtradeno;?>&doc_id=<?php echo $doc_id;?>&total=<?php echo $total;?>&status=<?php echo $status;?>&hz_id=<?php echo $hz_id;?>&person_id=<?php echo $person_id;?>";
					}
	...
	function confirmpay()
	{
		...
		else
		{
			...
			{
				var pwd = prompt("请输入你的储值卡密码:","");
				if(pwd==null){
					return false;
				}
				if(pwd=="")
				{
					alert("请输入你的储值卡密码！"); return false;
				}
				$.ajax(
				{
					type: "post",
...
<div class="zhifu_bot">
	...
	<ul>
		...
		<li id="czk">
			...
			<div class="chongzhika"><span class="card">储值卡</span> 
			<?php
			if($card_number)
			{
			...
			{
				echo "<font style='color:#EF2525;font-size:12px'>请您到当地医院内办储值卡，或到店支付</font>";
			}
			?>
			</div>
...
<div class="queding" style=" margin-top:0;"><a href="javascript:void(0)" onclick="confirmpay()">确认支付</a></div>
