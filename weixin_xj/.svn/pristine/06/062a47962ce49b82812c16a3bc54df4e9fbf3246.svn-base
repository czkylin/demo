...
class ReferralController extends Controller
{
	...
	public function referral()
	{
		...
		$url = C("PATH_URL")."/interface/yc_doc/referral_hos_list.aspx?access_token=".$token;
		$data = array
		(
			"openid"=>"$openid"
		);
	...
	{	
		...
		$url = C("PATH_URL")."/interface/yc_doc/referral_info.aspx?access_token=".$token;
		$data = array
		(
			"openid"=>"$openid",
	...
	public function referral_add()
	{
		...
		$url = C("PATH_URL")."/interface/yc_doc/referral_add.aspx?access_token=".$token;
		$data = array
		(
			"openid"=>"$openid",
		...
		{
			echo "<script>alert('提交成功'); location.href='/weixin/index.php?m=Expert&c=Referral&a=referral_list'</script>";
		}
		else
		{		
	...
	public function referral_list()
	{
		...
		$url = C("PATH_URL")."/interface/yc_doc/referral_list.aspx?access_token=".$token;
		$data = array
		(
			"openid"=>"$openid",
			"page_num"=>"1"
	...
	//转诊审核列表
	public function referral_dlist()
	{
		//获取平台的access_token
		$token = A_token();
		...
		$url = C("PATH_URL")."/interface/yc_doc/referral_dlist.aspx?access_token=".$token;
		$data = array
		(
			"openid"=>"$openid",
			"page_num"=>"1"
	...
	//转诊审核列表加载更多
	public function referral_dlist_append()
	{
		$page_num = I('get.page_num','2');
		//获取平台的access_token
		$token = A_token();
	...
	//转诊审核详情
	public function referral_dinfo()
	{
		$referral_id = I('get.referral_id','');
		//获取平台的access_token
		$token = A_token();
	...
	//转诊审核提交
	public function referral_dinfo_add()
	{
		$referral_id = I('post.referral_id','');
		$agree = I('post.agree','');
		...
		$url = C("PATH_URL")."/interface/yc_doc/referral_reason_add.aspx?access_token=".$token;
		$data = array
		(
			"openid"=>"$openid",
			"referral_status"=>"$agree",
			"referral_id"=>"$referral_id",
			"expert_desc"=>"$reason"
		);
		$json=json_encode($data);
		...
		if($res['error_code'] == 'ok')
		{
			echo "<script>alert('审核成功'); location.href='/weixin/index.php?m=Expert&c=Referral&a=referral_dlist'</script>";
		}
		else
		{		
			switch ($res['error_code']) 
