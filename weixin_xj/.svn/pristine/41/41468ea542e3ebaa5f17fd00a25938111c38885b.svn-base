$(function() {
	...
	function volumeControl(e) {
		...
		if (percentage < 0) {
			percentage = 0;
			$('.otherControl .volume').attr('class', 'ui-icon-bugle volume glyphicon glyphicon-volume-off');
		}
		if (percentage > 50) {
			$('.otherControl .volume').attr('class', 'ui-icon-bugle volume glyphicon glyphicon-volume-up');
		}
		if (percentage > 0 && percentage <= 50) {
			$('.otherControl .volume').attr('class', 'ui-icon-bugle volume glyphicon glyphicon-volume-down');
		}
		if (percentage >= 100) {
			percentage = 100;
		}
	...
	function playControl() {
			...
			if (playVideo[0].paused) {
				playVideo[0].play();
				$('.playTip').removeClass('glyphicon-play').addClass('ui-icon-stop-active').fadeOut();
			} else {
				playVideo[0].pause();
				$('.playTip').removeClass('ui-icon-stop-active').addClass('glyphicon-play').fadeIn();
			}
		}
		//关灯
