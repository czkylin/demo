...
class Upload {
    ...
    public function upload($files='') {
        ...
        foreach ($files as $key => $file) {
            ...
            //android 图片上传编程二进制，获取不到后缀名，匹配类型 image%3A755696 直接指定jpg后缀 edit by 白星海
            $file_name = explode('%3A', $file['name']);
            if($file['type'] == 'application/octet-stream' && $file_name[0] == 'image' && is_numeric($file_name[1])){
                $file['ext'] = 'jpg';
            }
            // var_dump($file);die;
            /* 文件上传检测 */
