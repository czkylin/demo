...
class UserController extends Controller
{
	...
	public function mywork_house()
	{
		...
		//判断当前用户是否绑定过患者端
		$url = C("PATH_URL")."/interface/yc_hos/band_member.aspx?access_token=".$token;
		$data = array
		(
			"openid"=>"$openid",
		...
		$json=json_encode($data);
		$band = json_decode(post_json($url,$json),true);
		$isband=0;
		if(isset($band['band_flag']) && $band['band_flag']=='true'){
			$isband=1;
		}
		// var_dump($isband);
		$this->assign('band',$isband);// 分享需用自己信息
