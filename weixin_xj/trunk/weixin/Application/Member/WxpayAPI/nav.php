...
$input->SetBody('1212');  				//商品描述
$input->SetOut_trade_no(1111111); 	//订单号
$input->SetProduct_id(1111111); 	//订单号
$input->SetTotal_fee(1);				//支付总额
$input->SetTime_start(date("YmdHis"));		//订单开始时间
$input->SetTime_expire(date("YmdHis", time() + 1200));//订单失效时间 
$input->SetAttach("");						//商品标记 可不填

// $input->SetNotify_url("http://wx-heartorg.yk2020.com/weixin/Application/Member/WxpayAPI/wx_notify_bak.php");
$input->SetNotify_url("http://wx-heartorg.yk2020.com/weixin/Application/Member/WxpayAPI/wx_notify.php");
...
echo '<img alt="模式一扫码支付" src="qrcode.php?data='.urlencode($order['code_url']).'" style="width:150px;height:150px;"/>';
var_dump($order['result_code']);die();
		...
		WeixinJSBridge.invoke(
			...
			function(res){
				...
				if(res.err_msg=="get_brand_wcpay_request:ok"){
					...
					{
						//去医院购买服务处理页面
						window.location.href="/weixin/index.php?m=Member&c=Hos&a=onlin_play&out_trade_no=<?php echo $out_trade_no;?>&outtradeno=<?php echo $outtradeno;?>&consult_id=<?php echo $consult_id;?>";
					}
					else if(<?php echo $status;?> == "2")
					{
						//去到会员的订单页面
						window.location.href="/weixin/index.php?m=Member&c=Cf&a=onlin_play&out_trade_no=<?php echo $out_trade_no;?>&outtradeno=<?php echo $outtradeno;?>&total=<?php echo $total;?>";
					}
					else if(<?php echo $status;?> == "3")
					{
						//去到会员的订单页面
						window.location.href="/weixin/index.php?m=Member&c=Yd&a=onlin_play&out_trade_no=<?php echo $out_trade_no;?>&outtradeno=<?php echo $outtradeno;?>&total=<?php echo $total;?>";
					}
					else
					{
						//去到会员的咨询页面
						window.location.href="/weixin/index.php?m=Member&c=Consult&a=onlin_play&out_trade_no=<?php echo $out_trade_no;?>&outtradeno=<?php echo $outtradeno;?>&doc_id=<?php echo $doc_id;?>&total=<?php echo $total;?>&status=<?php echo $status;?>&hz_id=<?php echo $hz_id;?>&person_id=<?php echo $person_id;?>&homeid=<?php echo $homeid;?>";
					}
		            ...
		            success: function(data)
                    {
                        ...
                        if(data['error_code']=="ok")
                        {
                            if(<?php echo $status;?> == "1")
                                {
                                    //去医院购买服务处理页面
                                    window.location.href="/weixin/index.php?m=Member&c=Hos&a=onlin_play&out_trade_no=<?php echo $out_trade_no;?>&consult_id=<?php echo $consult_id;?>&outtradeno="+data['r_tranno'];
                                }
                                else if(<?php echo $status;?> == "2")
                                {
                                    //去到会员的订单页面
                                    window.location.href="/weixin/index.php?m=Member&c=Cf&a=onlin_play&out_trade_no=<?php echo $out_trade_no;?>&total=<?php echo $total;?>&outtradeno="+data['r_tranno'];
                                }
                                else if(<?php echo $status;?> == "3")
                                {
                                    //去到会员的订单页面
                                    window.location.href="/weixin/index.php?m=Member&c=Yd&a=onlin_play&out_trade_no=<?php echo $out_trade_no;?>&total=<?php echo $total;?>&outtradeno="+data['r_tranno'];
                                }
                                else 
								{
									//去到会员的咨询页面
									window.location.href="/weixin/index.php?m=Member&c=Consult&a=onlin_play&out_trade_no=<?php echo $out_trade_no;?>&outtradeno="+data['r_tranno']+"&doc_id=<?php echo $doc_id;?>&total=<?php echo $total;?>&status=<?php echo $status;?>&hz_id=<?php echo $hz_id;?>&person_id=<?php echo $person_id;?>";
								}
                        }else
                        {
