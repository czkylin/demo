...
class JixiaoController extends Controller
{
	...
	public function add_info()
	{
		...
		if($_SESSION['lat'] == "")
		{
			$this->redirect('?m=Member&c=jixiao&a=get_gps&type=add_info');
		}
		...
		//获取医生列表
		$url = "http://weixin.yk2020.com/interface/yc_member/waiqin_type.aspx?access_token=".$token;
		$data = array
		(
			"openid"=>"$openid"
		);
	...
	public function add_ok()
	{
		...
		$urll = "http://weixin.yk2020.com/interface/yc_member/waiqin_add.aspx?access_token=".$token;
		$data = array
		(
			"openid"=>"$openid",
		...
		{
			//跳转到成功页面
			$this->redirect("?m=Member&c=Jixiao&a=success");
		}
		else
		{
			$msg = "信息错误";
	...
	public function get_gps($type)
	{
		...
		$url_str = "m=Member&c=Jixiao&a=".$type;
	...
	{  echo "dsds";die;
		...
		//获取绩效列表
		$url = "http://weixin.yk2020.com/interface/yc_member/waiqin_list.aspx?access_token=".$token;
		$data = array
		(
			"openid"=>"$openid",
			"page_num"=>"1"
	...
	public function jx_org_list()
	{
		...
		//获取医生列表
		$url = "http://weixin.yk2020.com/interface/yc_member/waiqin_org_list.aspx?access_token=".$token;
		$data = array
		(
			"openid"=>"$openid",
			"page_num"=>"1"
